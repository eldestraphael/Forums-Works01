...
import UserProfile from './user_id';
...
function Page(props: any) {
    const [viewUserAction, setViewUserAction] = useState<any>([]);
        ...
        try {
            ...
            if (res.status == 200) {
                ...
                api_data?.data?.map((item: any) => {
                    if (item.sub_module === "view_user") {
                        setViewUserAction(item.actions)
                    }
                })
    ...
    if (viewUserAction?.length) {
        return (
            <UserProfile page_props={props} viewUserAction={viewUserAction} />
        )
    } else {
        return (
