...
function Page(props: any) {
    const [addCompanyAction,  setAddCompanyAction] = useState<any>([]);
        ...
        try {
            ...
            if (res.status == 200) {
                ...
                api_data?.data?.map((item: any) => {
                    if (item.sub_module === "add_company") {
    ...
    if (addCompanyAction?.length > 0 ) {
        return (
            <AddCompany page_props={props} addCompanyAction={addCompanyAction} />
        )
    } else {
        return (
