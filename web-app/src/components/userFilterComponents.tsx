...
export default function UserFilter(props: any) {
    ...
    const filterAPIFunc = async () => {
        ...
        try {
            const res = await fetch(`/api/user/filter?search=`, {
                method: "GET",
                cache: 'no-store',
                headers: {
                    ...
                    <Grid container item xs={12} direction={'column'} >
                        ...
                        <Grid container item xs={12} direction={'row'} justifyContent={'space-between'} >
                            ...
                            <Grid container item xs={4} justifyContent={'flex-end'} alignItems={'center'} >
                                <Link  >
                                    <Typography
                                        sx={{ cursor: 'pointer', color: "#5F83ED", fontWeight: "700", fontSize: "16px" }}
                                        onClick={() => { props.setSelCompanyFilter([]); props.setSelForumFilter([]); push(`${pathname}${searchParams.get('page') ? `?page=${searchParams.get('page')}` : `?page=1`}${searchParams.get('search') ? `&search=${searchParams.get('search')}` : ''}`) }}
                                    >
                                        Clear All
                                    </Typography>
                                </Link>
                                                        ...
                                                        {item?.map((list: any, index: number) => {
                                                            return (
                                                                <FormControlLabel
                                                                    ...
                                                                    name={list?.uuid}
                                                                    onChange={(e) => handleCheckBox(e, Object.keys(filtersearchData)[i])}
                                                                />
                                                            )
                                                        })}
                                                    </FormGroup>
