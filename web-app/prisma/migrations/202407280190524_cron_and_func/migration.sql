...
-- -- 2. Create or Replace function calculate_user_forum_health()
  ...
  -- Main query to calculate user forum health scores ONLY
	...
	,weeks_finished AS ( 
		SELECT
	        forum_id,
			course_id,
		FLOOR((EXTRACT(EPOCH FROM (mv.start_timestamp_var::date + CAST(f.meeting_time AS TIME)) - (fc.starting_date + CAST(f.meeting_time AS TIME))) - EXTRACT(EPOCH FROM (meeting_time::time)::interval)) / 432000) AS weeks_since_start
	    FROM
	        forum_course fc
	    JOIN
	...
	,total_action_steps AS (
		SELECT
			f.id as forum_id,
			CASE
				WHEN (fc.starting_date + CAST(f.meeting_time AS TIME)) > (mv.start_timestamp_var::date + CAST(f.meeting_time AS TIME) - INTERVAL '5 days')
				THEN
	                (SELECT 
						COUNT(*)
	...
	,user_action_submissions AS (
	    ...
	    LEFT JOIN
	        user_forum_action_step_status ufass ON action_steps.id = ufass.action_step_id
			AND ufass.forum_id = fc.forum_id
			AND ufass.user_id = uf.user_id
	    WHERE 
			chapters.order = wf.weeks_since_start + 1 AND 
			fc.is_current_course = TRUE
		GROUP BY 
	        fc.forum_id, ufass.user_id, ufass.action_step_id, tas.ttl_act_stp
	...
	,final_scores AS (
		...
		GROUP BY
			...
			, fc.forum_id
			, chapters.id
			, uas.action_step_id,  uas.ttl_act_stp, uas.user_action_submission_count
...
-- 3. Set up cron to run every 22nd and 48th minute triggering the function calculate_user_forum_health()
-- SELECT cron.schedule('22,48 * * * *', 'SELECT public.calculate_user_forum_health()');
