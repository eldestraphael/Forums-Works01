...
AS $BODY$
    ...
    BEGIN
        ...
        ,final_scores AS (
            ...
            -- LEFT JOIN because some weeks may not have Action Steps
            LEFT JOIN
        ...
        -- -- INSERT into MCQ options of each user
            INSERT INTO user_forum_healths (health_mcq_option_id, user_id, forum_id, "date", score)
                SELECT (CONCAT('9993',fnl_prework_score)::int), user_id, forum_id, mv_start_timestamp_var, fnl_prework_score FROM final_scores
                    UNION ALL
                SELECT (CONCAT('9992',fnl_actstp_score)::int), user_id, forum_id, mv_start_timestamp_var, fnl_actstp_score FROM final_scores
                    UNION ALL
